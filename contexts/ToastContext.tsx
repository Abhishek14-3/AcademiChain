...
const Toast: React.FC<ToastMessage & { onDismiss: (id: number) => void }> = ({ id, message, type, onDismiss }) => {
  ...
  const baseClasses = "flex items-center w-full max-w-xs p-4 mb-4 text-ledger-brown bg-aged-paper rounded-sm shadow-lg border border-ledger-brown/30 font-serif";
  const typeClasses = {
    success: "text-status-valid bg-status-valid/10",
    error: "text-status-invalid bg-status-invalid/10",
    info: "text-gold-trim bg-gold-trim/10"
  };
  const iconClasses = `inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-sm ${typeClasses[type]}`;
  ...
  return (
    <div className={baseClasses} role="alert">
        ...
        <button type="button" onClick={() => onDismiss(id)} className="ml-auto -mx-1.5 -my-1.5 bg-transparent text-ledger-brown/50 hover:text-ledger-brown rounded-lg focus:ring-2 focus:ring-gold-trim p-1.5 hover:bg-gold-trim/20 inline-flex h-8 w-8" aria-label="Close">
